...
function calculateMovementFlowScore(wristIndex, elbowIndex) {
  ...
  if (validWristPos.length < 5) return 50;
  ...
  for (let i = 2; i < validWristPos.length; i++) {
    ...
    // 팔꿈치 위치 변화의 가속도 계산
    const elbowDist1 = calculateDistance(motionHistory.keypoints[motionHistory.keypoints.length - i][elbowIndex], motionHistory.keypoints[motionHistory.keypoints.length - i - 1][elbowIndex]);
    const elbowDist2 = calculateDistance(motionHistory.keypoints[motionHistory.keypoints.length - i - 1][elbowIndex], motionHistory.keypoints[motionHistory.keypoints.length - i - 2][elbowIndex]);
