...
function displayMotionData(analysis) {
  ...
  // Get current selected stroke type
  const selectedStrokeType = strokeType.value;
  ...
  // Separate angle data from other motion data
  const angleData = motionData.filter(([key]) => key.includes('Angle') || key.includes('Rotation'));
  const otherData = motionData.filter(([key]) => !key.includes('Angle') && !key.includes('Rotation'));
  
  // Display selected stroke type prominently
  tableHTML += `
    <div class="selected-stroke-section">
      <h3>Selected Stroke: <span class="stroke-highlight">${selectedStrokeType.charAt(0).toUpperCase() + selectedStrokeType.slice(1)}</span></h3>
    </div>
  `;

  // Display angle data in a separate section
  if (angleData.length > 0) {
    tableHTML += `<div class="angle-data-section"><h3>Angle Measurements</h3>`;
    
    // Sort angle data by readability (for better organization)
    angleData.sort((a, b) => getReadableLabel(a[0]).localeCompare(getReadableLabel(b[0])));
    
    // Display angle data with enhanced visualization
    angleData.forEach(([key, value]) => {
      ...
      // Add indication for important angles based on stroke type
      const isKeyAngleForStroke = isImportantAngleForStroke(key, selectedStrokeType);
      const importantClass = isKeyAngleForStroke ? 'important-angle' : '';
      
      tableHTML += `
        <div class="data-row ${importantClass}">
  ...
  // Display other motion data
  if (otherData.length > 0) {
    tableHTML += `<div class="other-data-section">`;
    
    if (angleData.length > 0) {
      tableHTML += `<h3>Other Measurements</h3>`;
    }
...
/**
 * Determine if an angle is important for the selected stroke type
 */
function isImportantAngleForStroke(angleKey, strokeType) {
  const strokeAngles = {
    'forehand': ['shoulderToElbowAngle', 'elbowToWristAngle', 'hipKneeAngle', 'shoulderAlignment'],
    'backhand': ['shoulderToElbowAngle', 'elbowToWristAngle', 'hipKneeAngle', 'shoulderAlignment'],
    'serve': ['shoulderToElbowAngle', 'elbowToWristAngle', 'shoulderAlignment'],
    'volley': ['shoulderToElbowAngle', 'elbowToWristAngle', 'kneeAngle', 'shoulderAlignment'],
    'auto': ['shoulderToElbowAngle', 'elbowToWristAngle', 'hipKneeAngle', 'shoulderAlignment']
  };
  
  // Return true if the angle is important for the selected stroke type
  return strokeAngles[strokeType] && strokeAngles[strokeType].some(angle => angleKey.includes(angle));
}
...
function calculateAngle(p1, p2, p3) {
  ...
  if (!p1 || !p2 || !p3 || !Array.isArray(p1) || !Array.isArray(p2) || !Array.isArray(p3)) {
    console.error('Invalid points provided to calculateAngle:', p1, p2, p3);
    return 0;
  }
