...
function calculateAngle(pointA, pointB, pointC) {
  ...
  if (!pointA || !pointB || !pointC) {
    console.log('각도 계산 실패: 포인트 없음', {pointA, pointB, pointC});
    return null;
  }
  
  // x, y 속성이 없거나 숫자가 아닌 경우 처리 및 디버깅
  if (typeof pointA.x !== 'number' || typeof pointA.y !== 'number' ||
    ...
    console.log('각도 계산 실패: 유효하지 않은 좌표 형식', {
      pointA: {x: pointA.x, y: pointA.y},
      pointB: {x: pointB.x, y: pointB.y},
      pointC: {x: pointC.x, y: pointC.y}
    });
    return null;
  }
  
  // NaN, Infinity 값 확인 및 디버깅
  if (isNaN(pointA.x) || isNaN(pointA.y) || 
    ...
    console.log('각도 계산 실패: NaN 또는 Infinity 값', {
      pointA: {x: pointA.x, y: pointA.y},
      pointB: {x: pointB.x, y: pointB.y},
      pointC: {x: pointC.x, y: pointC.y}
    });
  ...
  try {
    ...
    if (isNaN(angleRadians)) {
      console.log('각도 계산 실패: NaN 결과', {angleRadians});
      return null;
    }
    ...
    // 최종 결과 확인
    const result = isNaN(angleDegrees) ? null : Math.round(angleDegrees);
    
    // 성공적으로 계산된 경우 결과를 콘솔에 출력 (테스트용)
    if (result !== null) {
      console.log('각도 계산 성공:', {
        points: {
          pointA: {x: pointA.x, y: pointA.y},
          pointB: {x: pointB.x, y: pointB.y},
          pointC: {x: pointC.x, y: pointC.y}
